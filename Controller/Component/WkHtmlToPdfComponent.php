 ...
 */ 
    ...
    public function createPdf($layout = null) 
    {
        ...
        { 
            ...
            $url = "http://{$_SERVER['HTTP_HOST']}".DS."{$controllerName}/getViewDump/{$fileName}";
            $output = TMP."output{$randomNumber}.pdf";

            $header = APP.'pdf'.DS.'header.html'; 
            $footer = APP.'pdf'.DS.'footer.html';

            exec("wkhtmltopdf --print-media-type --footer-html $footer --header-html $header --header-spacing 10 -B 20 -L 15 -R 15 -T 50 --page-size A4 {$url} {$output}");
        } 
